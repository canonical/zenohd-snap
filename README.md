...
To pass a config file explicitly, use `--config` (or `-c`);
auto-discovery is skipped when this flag is present:

```bash
zenohd --config /home/user/snap/zenohd/current/my-config.json5
```
